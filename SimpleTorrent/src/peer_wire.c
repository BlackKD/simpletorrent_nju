...
#define LOCK_PIECE         if (++pieces_requesting >= 5) pthread_mutex_lock(pieces_state_mutex);
#define UNLOCK_FILE        pthread_mutex_unlock(file_mutex);
#define UNLOCK_PEER        pthread_mutex_unlock(peer_pool_mutex);
#define UNLOCK_PIECE       if (pieces_requesting-- >= 5) pthread_mutex_unlock(pieces_state_mutex);
...
int send_bitfield(int connfd) {
	...
	*len = reverse_byte_orderi(x_len + 1);
	...
	printf("send_bitfield x_len:%d, content: ", x_len);
...
int which_piece_to_request() {
	...
	printf("Piece %d is chosen to be requested, its num in peers is: %d\n", min, min_val);
	return min;
}
...
/*
 * A thread
 * To request all the pieces, End-game stage included
 */
void *request_file(void *arg) {
	int min_rest_num = 5;
	rest_pieces_num = (globalArgs.isseed == 1) ? 0 : globalInfo.g_torrentmeta->num_pieces;

	while (globalArgs.isseed == 0 && rest_pieces_num > min_rest_num) { 
	...
	/* End Game */
	if (rest_pieces_num <= 0) return NULL;
	is_end_game = 1;
	printf("\n\n\n End Game \n\n\n");
...
static inline void handle_piece(int connfd, peer_t *p, int index, int begin, int block_len, char *block) {
	...
	if (is_end_game != 1) {// not end-game, set the block directly 
		...
		if (globalInfo.pieces_state_arr[index] == PIECE_REQUESTING) { // this peer's block came first
			set_block(index, begin, block_len, block);
			globalInfo.pieces_state_arr[index] = PIECE_REQUESTING + connfd;
		}
		else if (globalInfo.pieces_state_arr[index] == PIECE_REQUESTING + connfd) { // the same peer
			set_block(index, begin, block_len, block);
		}
		else { // other peer
			// send cancel
